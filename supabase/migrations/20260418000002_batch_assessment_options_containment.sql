-- Batch unit date filter: an instance matches when it lies inside the window
-- (start_date >= p_from_date AND end_date <= p_to_date), not when end equals "To" only.
-- Units returned for the UI = all forms linked to the batch course; eligibility still
-- uses batch students + course forms + date rules.
...
as $$
  ...
  eligible as (
    ...
    join course_units cu on cu.form_id = i.form_id
