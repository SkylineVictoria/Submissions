-- Soft-delete induction windows: keep skyline_inductions row + FK from submissions (no CASCADE delete).
-- Public flows ignore rows with deleted_at set.

ALTER TABLE skyline_inductions
  ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ NULL;

CREATE INDEX IF NOT EXISTS idx_skyline_inductions_active
  ON skyline_inductions (start_at DESC)
  WHERE deleted_at IS NULL;

COMMENT ON COLUMN skyline_inductions.deleted_at IS 'When set, window is removed from admin list and public link is disabled; submissions remain linked.';

-- Unlock: only non-deleted inductions
...
BEGIN
  ...
  INNER JOIN skyline_inductions i ON i.id = ses.induction_id
...
BEGIN
  ...
  FROM skyline_inductions
  WHERE id = p_induction_id AND deleted_at IS NULL;
  IF v_start IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Induction not found or removed.');
  END IF;
