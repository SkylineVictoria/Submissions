-- Unit list for the batch UI: only forms that appear in `eligible` (same date rules as students).
-- Removes the union with all course-linked forms so the dropdown matches the assessment time window.
